...
// creating empty arrays to seed data into
...
const addDepartment = () => {
    inquirer.prompt([
        ...
        departmentsArr.push(answers.name)
    ...
    for (i = 0; i < results.length; i++) {
       ...
       inquirer.prompt([
            ...
            db.query(`SELECT id FROM departments WHERE name=?`,[answers.department], function (err, depResults) {
                ...
                db.query(`INSERT INTO roles (title, salary, department_id) VALUES (?,?,?)`,[answers.title,answers.salary,`${depResults}`], function (err, newRole) {
...
const addEmployee = () => {
    ...
    inquirer.prompt([
        ...
        console.log(answers);
        employeesArr.push(answers.firstName);
        findRoleId(answers);
        findManagerId(answers);
        // display database table
        db.query(`INSERT INTO employees (first_name, last_name, role_id,manager_id) VALUES (?,?,?,?)`, [`${answers.firstName},${answers.lastName},${roleId},${departmentId},${managerId}`], function (err, results) {
            console.table(results);
          });
        //   needs to wait for db.query to finish before calling main
        main();
...
const findDepartmentId = (answers) => {
    db.query(`SELECT id FROM departments WHERE name=?`,[`${answers.department}`], function (err, depResults) {
        const departmentId = depResults;
        return departmentId;
    });
}

const findManagerId = (answers => {
    db.query(`SELECT id FROM employees WHERE first_name=?, last_name=?`,[`${answers.firstName},${answers.lastName}`], function (err, results) {
